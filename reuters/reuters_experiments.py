...
def load_data():
    ...
    X = X[:10000]
    # try mini batch normailizing
    print(X.shape)
    normalized_X = []
    for i in range(len(X) // 1000):
        temp = preprocessing.normalize(X[i * 1000: (i+1) *1000], norm='l2')*200
        normalized_X.append(temp)
    normalized_X.append(X[(i+1) *1000:])
    normalized_X = np.concatenate(normalized_X)
...
#X,Y = load_data()
reuters10k_data = io.loadmat("dataset/reuters10k/reuters10k.mat")
X,Y = reuters10k_data["X"], reuters10k_data["Y"]
Y = Y.reshape(-1)
original_dim = 2000
n_centroid = 4 
...
mean = sample_output.predict(X,batch_size=batch_size)
pred = np.argmax(p_c_z_output.predict(X,batch_size=batch_size),axis=1)
...
cluster_means = u_init.T[pred]
scaled_cluster_means = cluster_means * 3
scaled_mean = mean + cluster_means * 3

print("prediction done ...")

print("VaDE Trans DP:", compute_purity_average(scaled_mean, Y, 4, 1000, 100, "ward", True, VERBOSE = True))
print("VaDE DP:", compute_purity_average(mean, Y, 4, 1000, 100, "ward", VERBOSE = True))

print("VaDE MW:", compute_MW_objective_average(4, mean, Y, 1000, 100, "ward", VERBOSE = True))
print("VaDE Trans MW:", compute_MW_objective_average(4, scaled_mean, Y, 1000, 100, "ward", VERBOSE = True))
